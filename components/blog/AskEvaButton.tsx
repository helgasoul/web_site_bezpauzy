...
interface AskEvaButtonProps {
  articleTitle?: string
  articleSlug?: string
}

export const AskEvaButton: FC<AskEvaButtonProps> = ({ articleTitle, articleSlug }) => {
  // Build chat link with article context
  const chatLink = articleSlug
    ? `/chat?article=${articleSlug}`
    : '/chat'
      ...
      <div className="bg-gradient-to-br from-primary-purple/10 via-ocean-wave-start/10 to-warm-accent/10 rounded-3xl p-8 md:p-10 border-2 border-primary-purple/20 shadow-lg relative overflow-hidden">
        ...
        <div className="relative z-10">
          ...
          <p className="text-body text-deep-navy/80 mb-6 leading-relaxed">
            {articleTitle
              ? `Спросите Еву о "${articleTitle}" — она ответит с учётом вашей ситуации и даст персональные рекомендации.`
              : 'Спросите Еву в чате — она ответит на основе научных источников и вашей истории.'}
          </p>
